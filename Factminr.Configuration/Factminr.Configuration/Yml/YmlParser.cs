...
namespace Factminr.Configuration.Yml
{
    public class YmlParser
    {
        ...
        public YmlParser(string path)
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                var kv = Regex.Split(line, ":", RegexOptions.IgnoreCase);
                findPreSpace(line);
                Node node = new Node();
        ...
        public Dictionary<string, object> GetKvsByParentKey(string parentKey)
        {
            Node parent = FindNodeByKey(parentKey);
            ...
            if (children.Count > 0)
            {
                ...
                {
                    kvs.Add(child.name, child.value);
                }
            }
            return kvs;
        }
